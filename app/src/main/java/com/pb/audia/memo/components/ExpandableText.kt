...
fun ExpandableText(
    ...
    Text(
        text = textToShow,
        maxLines = if (isExpanded) Int.MAX_VALUE else collapsableMaxLines,
        modifier = modifier
            .fillMaxWidth()
            .clickable(
                enabled = isClickable,
        ...
        onTextLayout = { result ->
            if (!isExpanded && result.hasVisualOverflow) {
                isClickable = true
                lastCharacterIndex = result.getLineEnd(lineIndex = collapsableMaxLines - 1)
            }
        }
    )
